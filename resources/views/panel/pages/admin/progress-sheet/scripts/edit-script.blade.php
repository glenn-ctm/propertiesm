<script>
    function tableRowHandler() {
        const sheetFields = {!! json_encode(old('items', $nProgSheet)) !!};
        const itemFields = {!! json_encode(old('items', $items)) !!};
        const lsPerWeek =  JSON.parse({!! json_encode($nProgSheet['ls_per_week']) !!});
        const scPerWeek = JSON.parse({!! json_encode($nProgSheet['sc_per_week']) !!});
        const user = {!! $user !!};
        ...
        return {
            ...
            removeField(index) {
                if(!this.itemFields[index]?.newItem) {
                    this.removedItems.push(this.itemFields[index]);
                }
    ...
    function calculator(datas) {
        return {
            ...
            lsPerWeekTotal() {
                ...
                let lsAmountPerWeek = parseFloat(datas.sheetFields.ls_amount_per_week);
