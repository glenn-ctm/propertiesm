<script>
    function tableRowHandler() {
        const itemFields  = {!! json_encode(old('items', $progSheetItems)) !!};
        const sheetFields = {!! json_encode($nProgSheet) !!}
        const lsPerWeek   = {!! json_encode(old('lsPerWeek', $lsPerWeek)) !!}
        const scPerWeek   = {!! json_encode(old('scPerWeek', $scPerWeek)) !!}
        return {
            itemFields,
            lsPerWeek,
            scPerWeek,
            calculation: calculator({itemFields, sheetFields, lsPerWeek, scPerWeek}),
            addNewField() {
    ...
    function calculator(datas) {
        return {
            ...
            lsPerWeekTotal() {
                ...
                // let lsAmountPerWeek = parseFloat(sheetFields.landScaping);
                // let multiplier = 0;
                // let formatedLscpw = [];
                ...
                let lsAmountPerWeek = parseFloat(datas.sheetFields.landScaping);
                return this.moneyRound(lsAmountPerWeek);
            },
            scPerWeekTotal() {
                let { sheetFields, scPerWeek } = datas;
                let scAmountPerWeek = parseFloat(sheetFields.siteCheck);
                let multiplier = 0;
                let formatedScpw = [];
