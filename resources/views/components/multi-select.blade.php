...
<div x-data="dropdown{{$id}}()" x-init="loadOptions()" class="w-full flex flex-col mx-auto">
    <select class="hidden" id="select{{$id}}" multiple>
        @foreach($values as $value)
            <option @if( in_array( $value, $old_selected ) ) selected @endif value="{{ $value }}">{{ $value }}</option>
        @endforeach
    </select>
    <input type="hidden" name="{{$name}}" x-ref="selectedValues{{$id}}" :value="selectedValues()">
      <div class="inline-block relative w-full">
          <div class="flex flex-col items-center relative">
              <div x-on:click="open" class="w-full">
                  <div class="p-1 flex border border-gray-300 bg-white rounded bg-gray-100 flex rounded appearance-none outline-none w-full text-gray-800 focus:outline-none focus:shadow-outline focus:bg-white">
                      <div class="flex flex-auto flex-wrap">
                          ...
                          <div x-show="selected.length == 0" class="flex-1">
                              <input placeholder="Select Option/s" class="bg-transparent p-1 px-2 appearance-none outline-none h-full w-full text-gray-700 text-sm font-light" :value="selectedValues()">
                          </div>
                      </div>
